...
bool SBPComptonImaging::backProjection(TTree* eventtree)
{
  ...
  for (int ev=0; ev<nEvent; ev++) {
    ...
    // visibility calc
    TVector3 direction = axis;
    TVector3 artho = direction.Orthogonal();
    direction.Rotate(thetaE,artho);

    const int sampling = 1000;
    double count = 0.0;
    for(int s=0;s<sampling;s++){

      double distance = zcenter - ver.Z();
      double phi = 2.*TMath::Pi()*gRandom->Rndm();
      TVector3 sampl = direction;
      sampl.Rotate(phi,axis);
      sampl = (distance/std::fabs(sampl.Z())) * sampl;
      TVector3 position = sampl + ver;
      if(0.< position.Z() &&
	 m_ImageCenterX-0.5*m_ImageWidthX<position.X() &&  position.X()< m_ImageCenterX+0.5*m_ImageWidthX &&
	 m_ImageCenterY-0.5*m_ImageWidthY<position.Y() &&  position.Y()< m_ImageCenterY+0.5*m_ImageWidthY)
	{
	  count += 1.0;
	}
    ...
    double visibility = count/(double)sampling;
    ...
    for(int i=1;i<=nbinx;i++){
      for(int j=1;j<=nbiny;j++){
        ...
        if(mi<rcrit){
          ...
          double aa = (diri.Mag()) * TMath::Cos(chii)* TMath::Tan(sigma);
	  double value = 1.0/(TMath::Power((diri.Mag()) * TMath::Cos(chii),weight))*TMath::Gaus((diri.Mag()) * TMath::Sin(chii),0,aa);
           m_ImageArray[0]->AddBinContent(gbin,1./norm * visibility * value);
        }
